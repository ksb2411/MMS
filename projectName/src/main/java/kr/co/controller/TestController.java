	...
	public ResponseEntity<byte[]> downloadorshow(String linkurl) {
		...
		try {
			...
			if (mType != null) {
				...
				/* ==> OCTET_STREAM라는 mediatype을 가져야 다운로드가 가능하다 */
				...
				/* ==> _를 찾아 substring을 가져온뒤 header에 추가시켜 오리지널 파일명을 추출한다. 브라우저에 맞는 인코딩에 맞춰준다. */
				
			}

			entity = new ResponseEntity<byte[]>(IOUtils.toByteArray(in), headers, HttpStatus.OK);
			/* ==> file에 있는 내용을 byte 배열로 만들어주는 코드 */
